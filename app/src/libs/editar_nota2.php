<?php
/* VERSIÓN DEPRECIADA en favor de app/src/libs/editar_nota3.php
No se utiliza desde el 2014-05-09 */
$time = microtime();
$time = explode(' ', $time);
$time = $time[1] + $time[0];
$start = $time;
...
if(!empty($id_asignacion)){
	...
	if($grupo = $bd->obtener_fila($stmt_grupo, 0)){
		...
		$query_notas = "SELECT * FROM gn_nota WHERE id_asignacion=".$id_asignacion;
		...
		while($registro_nota = $bd->obtener_fila($stmt_notas, 0)){
			if($registro_nota["tipo"]=="2"){
				$query_calendario = "SELECT id, id_cr_asis_descripcion FROM gr_calendario WHERE id=".$registro_nota["id_gr_calendario"];
				...
				if($calendario = $bd->obtener_fila($stmt_calendario, 0)){
					...
					if($modulo["punteo_max"]>=$array_notas[$keys[$contador_nota]]){
						...
						array_push($respuesta["errores"], "No se alteró una nota de ".$registro["nombre"]." ".$registro["apellido"]." (Asignación: ".$registro["asignacion"]."; ingresó ".$array_notas[$keys[$contador_nota]].", el máximo permitido para M".$modulo["modulo_num"]." es ".$modulo["punteo_max"].")");
						$respuesta["estado"] = 0;
					}
				}
			}
