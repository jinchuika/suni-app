...
var cont = 0;
function nuevo_marcador(lat,lng,desc, cant) {
  ...
  var algo = "";
  $.each(cant, function (index, item) {
    cont = cont + Number(item.cant);
    algo = algo+item.cant+" "+item.tipo+"s<br />";
  });
  infos.push(new google.maps.InfoWindow({content: "<div id='div_info'><b>"+desc+"</b>, <br /> "+algo+"</div>",maxWidth: 200}));
...
function crear_mapa() {
  ...
  mapa_temp = new Array();
  $.getJSON("../../src/libs/listar_mapa_cap.php", function (json) {
    console.log(json);
    var cant_maestros = 0, cant_asig = 0;
    $.each(json, function(i, entry){
      nuevo_marcador(entry.lat,entry.lng,entry.info,entry.cant_maestros);
      cant_maestros = cant_maestros + Number(entry.cant_maestros.length);
      cant_asig = Number(entry.cant_asig);
    ...
    document.getElementById('cantidad').innerHTML = cont+ ' personas';
    document.getElementById('cantidad2').innerHTML = cant_asig;
  });
