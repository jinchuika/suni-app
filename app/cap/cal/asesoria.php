...
<head>
	...
	?>
	<meta charset="UTF-8">
	<title>Calendario de asesorías</title>
	...
	function editar_fecha (id_cal, fecha, hora_cambio) {
		...
		$.ajax({
			url: nivel_entrada+'app/src/libs/editar_asesoria.php?calendario=1',
			type: 'post',
			data:{
				name: 'evento',
				pk: id_cal,
	...
	function editar_asesoria (id_cal, hora_inicio, hora_fin, id_sede, id_grupo) {
		var formulario = '<form id="form_editar_asesoria" class="form-inline"><span class="help-inline">Hora de inicio</span><input name="v_hora_inicio" id="v_hora_inicio" value="" type="text" class="input-small" placeholder="00:00"><span class="help-inline">Hora de Fin</span><input name="v_hora_fin" id="v_hora_fin" value="" type="text" class="input-small" placeholder="23:59"><br><a href="'+nivel_entrada+'app/cap/sed/sede.php?id='+id_sede+'" target="_blank">Abrir sede</a></form>';
	...
	function crear_asesoria (id_sede, fecha, hora_inicio, hora_fin) {
		console.log(id_sede+"->"+fecha+"->"+hora_inicio+"->"+hora_fin);
		$.ajax({
			url: nivel_entrada+"app/src/libs/crear_asesoria.php?calendario=1",
			data: {
				id_sede: id_sede,
				fecha: fecha,
	...
	$(document).ready(function() {
		...
		$("#id_sede").change(function () {
			$("#nuevo_evento").attr('data-modulo', document.getElementById('id_sede').value);
		});
		...
		$('#calendar').fullCalendar({
			...
			drop: function(date, allDay) {
				...
				crear_asesoria(document.getElementById('id_sede').value,date.getFullYear()+"-"+(mes_evento)+"-"+dia_evento, date.getHours()+":"+date.getMinutes(), (date.getHours()+2)+":"+date.getMinutes());
				$("#calendar").fullCalendar('refetchEvents');
			},
			eventMouseover: function(calEvent, jsEvent, view) {
				...
				$this.popover({
					...
					html: true,
					content: "<p>" + calEvent.other.curso +"</p><small>" + calEvent.other.grupo +"</small>" 
				});
				$(this).popover('show');
			},
			...
			eventClick: function (event) {
				editar_asesoria(event.id_cal, event.h_ini, event.h_fin, event.id_sede, event.id_grupo);
				return false;
			},
			...
			eventSources: [
			{
				events: function (start, end, callback) {
					$.ajax({
						url: '../../src/libs/listar_asesorias.php?calendario=1',
					//dataType: 'json',
					type: 'get',
					data: {
