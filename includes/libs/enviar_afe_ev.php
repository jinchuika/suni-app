...
if(!empty($_GET['consulta'])){
	...
	$anno = $fechaF[2];
	$fecha = $anno."-".$mes."-".$dia;
	$sql = "SELECT count(afe_ev_cuerpo.id_afe_ev_cuerpo)
	FROM afe_ev_encabezado
	inner join afe_ev_cuerpo ON afe_ev_encabezado.id_afe_ev_encabezado = afe_ev_cuerpo.id_afe_ev_encabezado
	WHERE (fecha ='$fecha') AND (semana ='$semana') AND (capacitador='$nombre_usr') AND (jornada='$jornada') AND (grupo ='$grupo') AND (sede ='$sede') AND (depto ='$depto') AND (municipio ='$municipio')";
	$stmt=$bd->ejecutar($sql);
	$x=$bd->obtener_fila($stmt,0);
	echo json_encode($x);
}
